...
func (vs *DefaultGroupStore) tombstoneDiscardConfig(cfg *GroupStoreConfig) {
...
func (vs *DefaultGroupStore) tombstoneDiscardLaunch() {
...
// (deletion markers).
func (vs *DefaultGroupStore) DisableTombstoneDiscard() {
...
func (vs *DefaultGroupStore) EnableTombstoneDiscard() {
	c := make(chan struct{}, 1)
	vs.tombstoneDiscardState.notifyChan <- &backgroundNotification{
		enable:   true,
		doneChan: c,
...
func (vs *DefaultGroupStore) TombstoneDiscardPass() {
...
func (vs *DefaultGroupStore) tombstoneDiscardLauncher() {
	var enabled bool
...
func (vs *DefaultGroupStore) tombstoneDiscardPassLocalRemovals() {
...
func (vs *DefaultGroupStore) tombstoneDiscardPassExpiredDeletions() {
	// Each worker will perform a pass on a subsection of each partition's key
	// space. Additionally, each worker will start their work on different
	// partition. This reduces contention for a given section of the locmap.
	partitionShift := uint16(0)
	...
	work := func(partition uint64, worker uint64, localRemovals []groupLocalRemovalEntry) {
		...
		for more {
			...
			rangeBegin, more = vs.vlm.ScanCallback(rangeBegin, rangeEnd, _TSB_DELETION, _TSB_LOCAL_REMOVAL, cutoff, uint64(vs.tombstoneDiscardState.batchSize), func(keyA uint64, keyB uint64, nameKeyA uint64, nameKeyB uint64, timestampbits uint64, length uint32) bool {
			...
			for i := 0; i < localRemovalsIndex; i++ {
				...
				vs.write(e.keyA, e.keyB, e.nameKeyA, e.nameKeyB, e.timestampbits|_TSB_LOCAL_REMOVAL, nil, true)
			}
		}
	}
	...
	for len(vs.tombstoneDiscardState.localRemovals) <= int(workerMax) {
		vs.tombstoneDiscardState.localRemovals = append(vs.tombstoneDiscardState.localRemovals, make([]groupLocalRemovalEntry, vs.tombstoneDiscardState.batchSize))
	}
	wg := &sync.WaitGroup{}
	wg.Add(int(workerMax + 1))
	for worker := uint64(0); worker <= workerMax; worker++ {
