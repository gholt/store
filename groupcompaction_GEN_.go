...
func (store *DefaultGroupStore) compactFile(fullPath string, blockID uint32) {
	...
	fdc, errs := groupReadTOCEntriesBatched(fpr, blockID, freeBatchChans, pendingBatchChans, make(chan struct{}))
	closeIfCloser(fpr)
	for _, err := range errs {
		store.logError("compactFile: error with %s: %s", fullPath, err)
	}
