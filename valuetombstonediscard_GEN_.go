...
func (vs *DefaultValueStore) tombstoneDiscardConfig(cfg *ValueStoreConfig) {
...
func (vs *DefaultValueStore) tombstoneDiscardLaunch() {
...
func (vs *DefaultValueStore) tombstoneDiscardPassExpiredDeletions() {
	...
	for len(vs.tombstoneDiscardState.localRemovals) <= int(workerMax) {
		vs.tombstoneDiscardState.localRemovals = append(vs.tombstoneDiscardState.localRemovals, make([]valueLocalRemovalEntry, vs.tombstoneDiscardState.batchSize))
	}
	wg := &sync.WaitGroup{}
	wg.Add(int(workerMax + 1))
	for worker := uint64(0); worker <= workerMax; worker++ {
